...
test("installed Chromium executes the matrix-marked governed action subset and origin denial", { timeout: 60_000 }, async (t) => {
  ...
  const actions = [
    ...
    "upload", "click", "popup.wait", "navigate"
  ];
  ...
  await adapter.act({
    sessionId: opened.sessionId,
    action: { kind: "click", origin: app.origin, selector: "#popup" }
  }, bound);
  const popup = await adapter.act({
    sessionId: opened.sessionId,
    action: { kind: "popup.wait", origin: app.origin, timeoutMs: 5_000 }
  }, bound);
  assert.match(popup.data.url, /\/popup$/);
