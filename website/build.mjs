      ...
      "@graph": [
        ...
        },
        {
          ...
          mainEntity: [
            ...
            faqSchema("Does it provide GitHub, YouTube, X, or Reddit access?", "The stable 0.2.0 package does not. The 0.3.0-alpha.3 prerelease adds public GitHub REST, official provider adapters, a pinned no-key YouTube route, optional read-only X and Reddit session routes, ordered provider fallback, and explicit doctor output. It does not extract cookies or expose social write operations.")
          ]
        }
      ]
    }
...
function footer() {
  return `
    <footer class="site-footer">
      ...
      <div class="shell legal"><span>MIT · npm stable 0.2.0 · npm next 0.3.0-alpha.3</span><span>Site content last reviewed 21 July 2026</span></div>
    </footer>`;
}
  ...
  return `
    <section class="hero shell">
      <div class="hero-copy">
        ...
        <div class="candidate-note"><span>Published prerelease</span><p><strong>0.3.0-alpha.3</strong> adds ordered source routing, optional read-only reach providers, a governed browser-host contract, and the existing provider and serverless surfaces under npm <code>next</code>. Stable users remain on 0.2.0.</p></div>
      </div>
    ...
    <section class="section shell capability-section" aria-labelledby="capability-title">
      <div class="capability-intro">
        ...
        ${codeBlock("home-doctor", "capability check", "npx -y --package cockroach-crawler@0.3.0-alpha.3 cockroach-sources doctor\nnpx -y --package cockroach-crawler@0.3.0-alpha.3 cockroach-reach doctor")}
    ...
    <section class="section shell faq-section"><div><p class="eyebrow">Questions answered plainly</p><h2>Know the limits before installing.</h2></div><div class="faq-list">
      ...
      <details><summary>Can it read GitHub, YouTube, X, or Reddit?</summary><p>The npm-stable 0.2.0 package has no provider registry. The 0.3.0-alpha.3 prerelease adds public GitHub REST, official provider adapters, a restricted no-key YouTube route, and optional read-only X and Reddit session providers. Session providers require explicit local installation and never expose posting, liking, following, messaging, deleting, cookie extraction, or profile-file import.</p></details>
      <details><summary>Can I run it in a serverless edge function?</summary><p>The 0.3.0-alpha.3 prerelease includes a self-hosted, token-authenticated, rate-limited Cloudflare Worker profile for deployment-configured HTTPS origins. It is bounded, but it does not resolve, classify, or pin DNS answers; an allowlisted hostname can resolve internally. Use operator-owned or independently trusted hostnames plus infrastructure egress policy. Stable users remain on 0.2.0.</p></details>
    </div></section>`;
}
...
const result = await crawlTool.execute({
  ...
  maxPages: 5
        ...
        <section id="sources"><p class="eyebrow">05 · Published prerelease · 0.3.0-alpha.3</p><h2>Inspect provider capability before dispatch.</h2><div class="callout candidate"><strong>npm next</strong><p>This contract is published under the npm <code>next</code> tag and is not in stable 0.2.0. Pin the exact prerelease version before using the package import.</p></div>${codeBlock("sources-doctor", "npm prerelease", "npm install cockroach-crawler@0.3.0-alpha.3\nnpx cockroach-sources doctor --json\nnpx cockroach-reach doctor --json")}${codeBlock("sources-example", "sources.mjs", `import { createSourceRegistryFromEnv } from "cockroach-crawler/sources";
...
console.log(repositories);`, "javascript")}<p>Public GitHub REST is ready with optional token authentication. YouTube metadata reads work through public oEmbed; search needs <code>YOUTUBE_API_KEY</code> and transcripts remain unavailable. X requires <code>X_BEARER_TOKEN</code>. Reddit requires official client credentials and a contact-aware user agent.</p></section>
        <section id="serverless"><p class="eyebrow">06 · Serverless prerelease · 0.3.0-alpha.3</p><h2>A smaller edge boundary with named tradeoffs.</h2><p>The prerelease includes a self-hosted Cloudflare Worker entry point. It accepts only token-authenticated <code>POST /v1/crawl</code>, requires configured HTTPS origins, and is rate-limited by the deployment.</p>${codeBlock("serverless-config", "worker/wrangler.jsonc", `{
  "name": "cockroach-crawler-serverless",
  "main": "worker.js",
...
function providersPage() {
  return `
    <section class="page-hero shell"><p class="eyebrow">Provider coverage · stable vs prerelease</p><h1>Know what is public, keyed, session-backed, and still missing.</h1><p class="lede">npm stable 0.2.0 ships the hardened web crawler. The 0.3.0-alpha.3 prerelease adds a tested provider registry, ordered routing, optional read-only reach providers, a governed browser-host contract, and a separate serverless tier under the <code>next</code> tag.</p><div class="page-actions"><a class="button primary" href="/docs/#sources">Inspect the prerelease API</a><a class="button secondary" href="${repository}/issues">Propose an adapter</a></div></section>
    ...
    <section class="section shell"><div class="table-wrap" tabindex="0" role="region" aria-label="Provider capability status table"><table class="status-table"><thead><tr><th>Surface</th><th>npm stable 0.2.0</th><th>npm next 0.3.0-alpha.3</th><th>Boundary</th></tr></thead><tbody>
