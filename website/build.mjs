...
const pages = [
      ...
      "@graph": [
        ...
        },
        {
          ...
          mainEntity: [
            ...
            faqSchema("Does it provide GitHub, YouTube, X, or Reddit access?", "The stable 0.2.0 package under npm latest does not. The published 0.3.0-alpha.1 prerelease under npm next adds a tested provider registry: public GitHub REST is ready, YouTube metadata reads work without a key while search needs a key, and X and Reddit require official operator credentials. YouTube transcripts are not implemented.")
          ]
        }
      ]
    }
  ...
  },
  {
    ...
    description: "Inspect and use the prerelease GitHub, YouTube, X, and Reddit provider contracts without hiding credential requirements.",
  ...
  },
  {
    ...
    description: "A precise stable-versus-prerelease status matrix for public HTTP, serverless crawling, GitHub, YouTube, Reddit, X, authentication, and transcripts.",
    body: providersPage()
  },
  {
    slug: "benchmark",
...
function footer() {
  return `
    <footer class="site-footer">
      ...
      <div class="shell legal"><span>MIT · npm latest: stable 0.2.0 · npm next: prerelease 0.3.0-alpha.1</span><span>Site content last reviewed 19 July 2026</span></div>
    </footer>`;
}
  ...
  return `
    <section class="hero shell">
      <div class="hero-copy">
        ...
        <ul class="signal-list" aria-label="Release facts"><li>npm latest: stable 0.2.0</li><li>Node 20.18+</li><li>MIT</li><li>No signup</li></ul>
        <div class="candidate-note"><span>Published prerelease · npm next</span><p><strong>0.3.0-alpha.1</strong> adds tested provider and serverless contracts. It is published for explicit alpha testing, not promoted to stable or deployed as a hosted crawler.</p></div>
      </div>
    ...
    <section class="section shell faq-section"><div><p class="eyebrow">Questions answered plainly</p><h2>Know the limits before installing.</h2></div><div class="faq-list">
      ...
      <details><summary>Can it read GitHub, YouTube, X, or Reddit?</summary><p>The stable 0.2.0 package under npm <code>latest</code> has no provider registry. The published 0.3.0-alpha.1 prerelease under <code>next</code> adds public GitHub REST, YouTube metadata, keyed YouTube search, official X API v2 bearer access, and Reddit application-only OAuth. YouTube transcripts are not implemented.</p></details>
      <details><summary>Can I run it in a serverless edge function?</summary><p>The published 0.3.0-alpha.1 prerelease includes a self-hosted, token-authenticated, rate-limited Cloudflare Worker profile for deployment-configured HTTPS origins. The package supplies a deployment template, not a hosted service. It is bounded, but it does not resolve, classify, or pin DNS answers; an allowlisted hostname can resolve internally. Use operator-owned or independently trusted hostnames plus infrastructure egress policy.</p></details>
    </div></section>`;
}
...
function providerDocsPage() {
  return focusedDocsPage(
    ...
    "The published prerelease reports whether each adapter is public, keyed, credentialed, partial, or unavailable without serializing secrets.",
    `<section><p class="eyebrow">01 · Doctor</p><h2>Inspect the current runtime.</h2>${codeBlock("provider-doctor-guide", "npm next", "npm install cockroach-crawler@next\nnpx cockroach-sources doctor --json")}</section>
...
const result = await crawlTool.execute({
  ...
  maxPages: 5
        ...
        <section id="sources"><p class="eyebrow">05 · Published prerelease · 0.3.0-alpha.1</p><h2>Inspect provider capability before dispatch.</h2><div class="callout candidate"><strong>Install from npm next</strong><p>This contract is published in 0.3.0-alpha.1 under npm <code>next</code>, but it is not part of stable 0.2.0 under <code>latest</code>. Pin the exact version for reproducible alpha testing.</p></div>${codeBlock("sources-doctor", "npm prerelease", "npm install cockroach-crawler@0.3.0-alpha.1\nnpx cockroach-sources doctor --json")}${codeBlock("sources-example", "sources.mjs", `import { createSourceRegistryFromEnv } from "cockroach-crawler/sources";
...
console.log(repositories);`, "javascript")}<p>Public GitHub REST is ready with optional token authentication. YouTube metadata reads work through public oEmbed; search needs <code>YOUTUBE_API_KEY</code> and transcripts remain unavailable. X requires <code>X_BEARER_TOKEN</code>. Reddit requires official client credentials and a contact-aware user agent.</p></section>
        <section id="serverless"><p class="eyebrow">06 · Published prerelease · 0.3.0-alpha.1</p><h2>A smaller edge boundary with named tradeoffs.</h2><p>The published prerelease includes a self-hosted Cloudflare Worker entry point. It accepts only token-authenticated <code>POST /v1/crawl</code>, requires configured HTTPS origins, and is rate-limited by the deployment. Installing the package does not deploy a Worker.</p>${codeBlock("serverless-config", "worker/wrangler.jsonc", `{
  "name": "cockroach-crawler-serverless",
  "main": "worker.js",
...
const pages = await crawl({
  ...
  "fetchedAt": "2026-07-18T00:00:00.000Z"
        ...
        <section id="deployment"><p class="eyebrow">10 · Deployment</p><h2>Pick the tier whose boundary you can defend.</h2><p>The stable package uses Node networking and optional Playwright; run it in a controlled Node process, container, or CI job. The published prerelease's Cloudflare Worker is a separate fetch-only profile with an explicit weaker network guarantee. Do not describe the tiers as equivalent.</p><div class="next-links"><a href="/security/"><span>Next</span><strong>Review the security model →</strong></a><a href="/providers/"><span>Then</span><strong>Check provider coverage →</strong></a></div></section>
      </div>
    </div>`;
}
...
function providersPage() {
  return `
    <section class="page-hero shell"><p class="eyebrow">Provider coverage · stable vs prerelease</p><h1>Know what is public, keyed, and still missing.</h1><p class="lede">npm <code>latest</code> points to stable 0.2.0 with the hardened web crawler. npm <code>next</code> points to the published 0.3.0-alpha.1 prerelease with a tested provider registry and a separate serverless tier.</p><div class="page-actions"><a class="button primary" href="/docs/#sources">Inspect the prerelease API</a><a class="button secondary" href="${repository}/issues">Propose an adapter</a></div></section>
    <section class="section shell feature-stage"><figure><img src="/assets/provider-map.svg" width="720" height="560" alt="Provider coverage map showing stable public web crawling, prerelease GitHub and YouTube adapters, and credential-gated X and Reddit adapters" /><figcaption>Doctor status is capability-based: public GitHub REST is ready, YouTube is partial, and X/Reddit stay closed without official credentials.</figcaption></figure><div><p class="eyebrow">Know before dispatch</p><h2>Every adapter reports its exact access state.</h2><p>Each adapter reports whether access is public, keyed, or credentialed before dispatch, together with its rate-limit and data-shape contract.</p><div class="candidate-note compact"><span>Published prerelease contract</span><p><code>cockroach-sources doctor --json</code> reports exact runtime status without serializing secrets.</p></div></div></section>
    <section class="section shell"><div class="table-wrap" tabindex="0" role="region" aria-label="Provider capability status table"><table class="status-table"><thead><tr><th>Surface</th><th>npm latest · stable 0.2.0</th><th>npm next · prerelease 0.3.0-alpha.1</th><th>Boundary</th></tr></thead><tbody>
      ...
      <tr><td>Serverless web tier</td><td><span class="status planned">Not shipped</span></td><td><span class="status conditional">Prerelease</span></td><td>Self-hosted, token-authenticated, rate-limited, operator-owned HTTPS allowlist; no DNS resolution/pinning or browser/social adapters.</td></tr>
...
function roadmapPage() {
  return `
    ...
    <section class="section shell roadmap-list">
      ...
      <article><div><span class="status conditional">Published prerelease · 0.3.0-alpha.1</span><h2>Provider registry</h2></div><ul><li>Web, GitHub, YouTube, X, and Reddit capability doctor</li><li>Public GitHub REST and public YouTube metadata reads</li><li>Official credential gates for keyed search and social APIs</li><li>Normalized records with provenance, hash, warnings, and adapter version</li></ul></article>
      <article><div><span class="status conditional">Published prerelease · 0.3.0-alpha.1</span><h2>Fetch-only serverless profile</h2></div><ul><li>Self-hosted Cloudflare Worker entry point</li><li>Bearer authentication, deployment rate limit, and operator-owned HTTPS origin allowlist</li><li>Exact runtime disclosure: no DNS resolution/pinning, browser, social providers, or request-selected arbitrary origins</li><li>Separate <code>cockroach-crawler/serverless</code> contract</li></ul></article>
      <article><div><span class="status planned">Next evidence</span><h2>Prerelease feedback and broader coverage</h2></div><ul><li>Collect reproducible provider and security feedback before any stable promotion</li><li>Keep npm <code>latest</code> on 0.2.0 until a separately reviewed stable artifact is published</li><li>Add transcript support only through a reviewed provider contract</li><li>Use benchmark distributions and raw evidence, not a single headline number</li></ul></article>
    </section>
...
await writeFile(
  ...
  `<?xml version="1.0" encoding="UTF-8"?>\n<urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9">\n${pages.map((page) => `  <url><loc>${siteUrl}${page.slug ? `/${page.slug}/` : "/"}</loc><lastmod>2026-07-19</lastmod></url>`).join("\n")}\n</urlset>\n`,
  "utf8"
);
