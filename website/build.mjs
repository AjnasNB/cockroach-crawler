...
const benchmarkResult = JSON.parse(
  await readFile(join(root, "..", "bench", "results", "local-regression.json"), "utf8")
);
...
function nav(active) {
  ...
  const secondary = pages.filter((page) => ["Roadmap", "Community", "Release"].includes(page.nav));
  const link = (page) => {
  ...
  return `
    <footer class="site-footer">
      <div class="shell footer-grid">
        ...
        <div><h2>Use</h2><a href="/docs/">Documentation</a><a href="/providers/">Provider status</a><a href="${npmPackage}">npm package</a></div>
        ...
        <div><h2>Project</h2><a href="/roadmap/">Roadmap</a><a href="/community/">Contribute</a><a href="${repository}">Source code</a></div>
      </div>
...
function codeBlock(id, label, code, language = "shell") {
  return `<div class="code-panel">
    ...
    <pre><code id="${id}" data-language="${language}">${escapeHtml(code)}</code></pre>
  ...
  return `
    ...
    <section class="section shell split-intro">
      ...
      <div class="mode-list">
        ...
        <article><span>02</span><div><h3>JavaScript API</h3><p>Typed options, callbacks, abort signals, failures, and crawl statistics.</p><a href="/docs/#library">See the API →</a></div></article>
        <article><span>03</span><div><h3>Agent adapter</h3><p>Model input cannot expand the origins, budgets, or browser authority set by its creator.</p><a href="/docs/#agent">Bind a tool →</a></div></article>
      </div>
    </section>
...
function docsPage() {
  return `
    ...
    <div class="docs-layout shell">
      <aside class="toc"><nav aria-label="On this page"><h2>On this page</h2><a href="#quickstart">Quickstart</a><a href="#cli">CLI</a><a href="#library">JavaScript API</a><a href="#agent">Agent adapter</a><a href="#sources">Source registry</a><a href="#serverless">Serverless</a><a href="#browser">Browser mode</a><a href="#output">Output</a><a href="#limits">Limits</a><a href="#deployment">Deployment notes</a></nav></aside>
...
const pages = await crawl({
  ...
  "fetchedAt": "2026-07-18T00:00:00.000Z"
        ...
        <section id="limits"><p class="eyebrow">09 · Limits</p><h2>Budget every dimension that can grow.</h2><div class="table-wrap"><table><thead><tr><th>Concern</th><th>Controls</th><th>Why it matters</th></tr></thead><tbody>
...
function providersPage() {
  return `
    ...
    <section class="section shell feature-stage"><figure><img src="/assets/provider-map.svg" width="720" height="560" alt="Provider coverage map showing released public web crawling, candidate GitHub and YouTube adapters, and credential-gated X and Reddit adapters" /><figcaption>Doctor status is capability-based: public GitHub REST is ready, YouTube is partial, and X/Reddit stay closed without official credentials.</figcaption></figure><div><p class="eyebrow">Read every row literally</p><h2>One registry does not mean universal access.</h2><p>Each adapter has a separate authentication, rate-limit, data-shape, and terms boundary. The registry reports capability before dispatch; it does not steal browser sessions, automate logins, or bypass platform policy.</p><div class="candidate-note compact"><span>Candidate contract</span><p><code>cockroach-sources doctor --json</code> reports exact runtime status without serializing secrets.</p></div></div></section>
    <section class="section shell"><div class="table-wrap"><table class="status-table"><thead><tr><th>Surface</th><th>npm stable 0.2.0</th><th>0.3.0-alpha.1 source candidate</th><th>Boundary</th></tr></thead><tbody>
...
function benchmarkPage() {
  return `
    <section class="page-hero shell"><p class="eyebrow">Local fixture benchmark · development capture</p><h1>A regression signal, not an internet-speed claim.</h1><p class="lede">On 18 July 2026, ${benchmarkMeasuredRuns} measured runs of the deterministic ${benchmarkPages}-page local fixture produced a ${benchmarkElapsedMedian} ms median and ${benchmarkThroughputMedian} pages per second median on this development machine.</p><div class="page-actions"><a class="button primary" href="#reproduce">Reproduce it</a><a class="button secondary" href="${repository}/blob/main/docs/BENCHMARK.md">Read the method</a></div></section>
    <section class="benchmark-hero shell"><div class="benchmark-number"><span>${benchmarkMeasuredRuns}-run median</span><strong>${benchmarkThroughputMedian}</strong><em>local fixture pages / second</em></div><figure><img src="/assets/benchmark-rig.svg" width="720" height="520" alt="Isometric benchmark rig feeding 120 local pages through a controlled extraction path into measured output" /><figcaption>0.3.0-alpha.1 working tree · ${benchmarkNode} · static local HTML · no public network · no Chromium · no competitor comparison</figcaption></figure></section>
    <section class="section shell"><div class="section-head"><div><p class="eyebrow">Scope</p><h2>What this number means.</h2></div><p>The benchmark is useful when repeated on comparable source and hardware; it is not a global standard.</p></div><div class="fit-grid"><article class="fit-yes"><span>It measures</span><h3>Local crawler regression</h3><p>Traversal and extraction across 120 deterministic local pages, including Markdown output and a likely-private URL skip.</p></article><article class="fit-no"><span>It does not measure</span><h3>Internet capacity</h3><p>No DNS latency, TLS, remote server pacing, robots delays, browser rendering, proxy network, or production queue.</p></article><article class="fit-no"><span>It does not claim</span><h3>Competitor ranking</h3><p>No matched Firecrawl, Crawl4AI, Crawlee, or Scrapy workload was run.</p></article></div></section>
    <section class="section shell proof-section" id="reproduce"><div><p class="eyebrow">Reproduce</p><h2>Run the committed fixture.</h2><p>Record the commit, Node version, CPU, operating system, and repeated-run distribution before publishing a comparison.</p>${codeBlock("bench-run", "terminal", "git clone https://github.com/AjnasNB/cockroach-crawler.git\ncd cockroach-crawler\nnpm ci --ignore-scripts\nnpm run bench")}</div>${codeBlock("bench-output", "captured 18 July 2026", `{
  ...
  "policyProbes": "passed"
    ...
    <section class="section shell"><div class="callout warning"><strong>Development evidence, not release evidence</strong><p>The captured JSON records a dirty working tree at baseline commit <code>${benchmarkCommit}</code>. It is useful for local regression tracking, but it must be regenerated from the approved release commit before it is cited as release evidence.</p></div></section>
...
await writeFile(join(dist, "_headers"), `/*\n  Content-Security-Policy: default-src 'self'; script-src 'self' 'unsafe-inline'; style-src 'self'; img-src 'self' data:; font-src 'self'; connect-src 'self'; object-src 'none'; base-uri 'none'; form-action 'self'; frame-ancestors 'none'\n  X-Content-Type-Options: nosniff\n  Referrer-Policy: strict-origin-when-cross-origin\n  X-Frame-Options: DENY\n  Permissions-Policy: camera=(), microphone=(), geolocation=(), payment=(), usb=()\n\n/assets/*\n  Cache-Control: public, max-age=86400\n`, "utf8");
