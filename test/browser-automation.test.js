...
test("generated capability matrix separates catalog, handlers, services, and real-engine verification", () => {
  ...
  assert.equal(matrix.summary.builtInHandlerActions, 61);
  assert.equal(matrix.summary.trustedServiceRequiredActions, 11);
  assert.equal(matrix.summary.explicitlyUnsupportedActions, 30);
  assert.equal(matrix.summary.realEngineIntegrationVerifiedActions, 29);
