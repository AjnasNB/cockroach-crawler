...
test("concrete backend advertises only mapped handlers and names unsupported families", () => {
  ...
  const backend = createGovernedPlaywrightBackend(config.services);
  assert.equal(backend.supportedActions.length, 72);
  assert.equal(GOVERNED_ENGINE_HANDLER_ACTIONS.length, 93);
  assert.equal(GOVERNED_ENGINE_UNSUPPORTED_ACTIONS.length, 30);
...
test("a delayed request authorization cannot outlive its bound action lease", async () => {
  ...
  assert.equal(config.calls.networkAborts, 1);
  await harness.adapter.closeSession({ sessionId: harness.opened.sessionId }, harness.bound);
});

test("a recoverable popup timeout closes the action network window", async () => {
  const config = fixture();
  const harness = await adapterFixture(config, ["popup.wait"], []);
  await assert.rejects(
    () => harness.adapter.act({
      sessionId: harness.opened.sessionId,
      action: { kind: "popup.wait", origin: ORIGIN, timeoutMs: 1 }
    }, harness.bound),
    (error) => error.code === "BROWSER_AUTOMATION_EVENT_NOT_READY"
  );
  assert.equal(config.calls.offline.at(-1), true);
  await config.calls.routes[0].handler({
    request() {
      return { url: () => `${ORIGIN}/after-popup-timeout`, method: () => "GET", resourceType: () => "fetch" };
    },
    async abort() { config.calls.networkAborts += 1; },
    async continue() { throw new Error("closed popup lease must not continue"); },
    async fallback() { throw new Error("closed popup lease must not continue"); }
  });
  assert.equal(config.calls.networkAborts, 1);
  await harness.adapter.closeSession({ sessionId: harness.opened.sessionId }, harness.bound);
});

test("closed queued popups are pruned before a later bounded wait", async () => {
  const config = fixture();
  const harness = await adapterFixture(config, ["popup.wait"], []);
  ...
  assert.equal(config.calls.closes, 0);
  await harness.adapter.closeSession({ sessionId: harness.opened.sessionId }, harness.bound);
});
...
test("an invalid factory isolation attestation closes the returned owned context", async () => {
  ...
  config.services.createSession = async () => ({
    ...
    page: config.page,
    ownedContext: true,
    networkIsolation: { serviceWorkers: "allow", webSockets: "block" }
  });
  const backend = createGovernedPlaywrightBackend(config.services);
  const adapter = createGovernedBrowserAutomation({
    backend,
...
test("page, frame, and worker bursts cross hard ceilings and quarantine the context", async () => {
  const pages = fixture();
  await adapterFixture(pages, ["popup.wait"], ["read"]);
  ...
  assert.equal(excessPageCloses, 1);
