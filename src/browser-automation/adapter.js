...
const DEFAULT_POLICY = Object.freeze({
  ...
  maxNetworkRequestBytes: 1024 * 1024,
  maxNetworkResponseBytes: 16 * 1024 * 1024,
  maxTotalNetworkRequestBytes: 8 * 1024 * 1024,
  maxTotalNetworkResponseBytes: 64 * 1024 * 1024
});
...
function normalizePolicy(value = {}) {
  const raw = exactRecord(
    ...
    new Set([
      ...
      "maxTotalUploadBytes", "maxNetworkRequestBytes", "maxNetworkResponseBytes",
      "maxTotalNetworkRequestBytes", "maxTotalNetworkResponseBytes"
    ]),
    new Set(),
  ...
  return Object.freeze({
    ...
    maxNetworkRequestBytes: raw.maxNetworkRequestBytes === undefined ? DEFAULT_POLICY.maxNetworkRequestBytes
      : normalizePositiveInteger(raw.maxNetworkRequestBytes, "Browser automation policy.maxNetworkRequestBytes", 256 * 1024 * 1024),
    maxNetworkResponseBytes: raw.maxNetworkResponseBytes === undefined ? DEFAULT_POLICY.maxNetworkResponseBytes
      : normalizePositiveInteger(raw.maxNetworkResponseBytes, "Browser automation policy.maxNetworkResponseBytes", 256 * 1024 * 1024),
    maxTotalNetworkRequestBytes: raw.maxTotalNetworkRequestBytes === undefined ? DEFAULT_POLICY.maxTotalNetworkRequestBytes
      : normalizePositiveInteger(raw.maxTotalNetworkRequestBytes, "Browser automation policy.maxTotalNetworkRequestBytes", 1024 * 1024 * 1024),
    maxTotalNetworkResponseBytes: raw.maxTotalNetworkResponseBytes === undefined ? DEFAULT_POLICY.maxTotalNetworkResponseBytes
      : normalizePositiveInteger(raw.maxTotalNetworkResponseBytes, "Browser automation policy.maxTotalNetworkResponseBytes", 1024 * 1024 * 1024)
  });
}
...
function authorityMatches(actual, expected) {
  return actual.authorityId === expected.authorityId
    ...
    && actual.maxNetworkRequestBytes === expected.maxNetworkRequestBytes
    && actual.maxNetworkResponseBytes === expected.maxNetworkResponseBytes
    && actual.maxTotalNetworkRequestBytes === expected.maxTotalNetworkRequestBytes
    && actual.maxTotalNetworkResponseBytes === expected.maxTotalNetworkResponseBytes
...
function publicSession(session) {
  return Object.freeze({
    ...
    networkBudget: Object.freeze({
      committedRequestBytes: session.committedNetworkRequestBytes,
      maximumRequestBytes: session.authority.maxTotalNetworkRequestBytes,
      committedResponseBytes: session.committedNetworkResponseBytes,
      maximumResponseBytes: session.authority.maxTotalNetworkResponseBytes
    }),
...
export function createGovernedBrowserAutomation(options) {
  ...
  async function openSession(request, authorityInput) {
    ...
    for (const [field, maximum] of [
      ...
      ["maxNetworkRequestBytes", policy.maxNetworkRequestBytes],
      ["maxNetworkResponseBytes", policy.maxNetworkResponseBytes],
      ["maxTotalNetworkRequestBytes", policy.maxTotalNetworkRequestBytes],
      ["maxTotalNetworkResponseBytes", policy.maxTotalNetworkResponseBytes]
    ]) {
    ...
    try {
      handle = await withinDeadline((signal) => backend.openSession(Object.freeze({
        ...
        networkBudget: Object.freeze({
          maxRequestBytesPerAction: boundAuthority.maxNetworkRequestBytes,
          maxResponseBytesPerAction: boundAuthority.maxNetworkResponseBytes,
          maxTotalRequestBytes: boundAuthority.maxTotalNetworkRequestBytes,
          maxTotalResponseBytes: boundAuthority.maxTotalNetworkResponseBytes
        }),
    ...
    const session = {
      ...
      committedNetworkRequestBytes: 0,
      committedNetworkResponseBytes: 0,
  ...
  async function act(request, authorityInput) {
    ...
    const run = async () => {
      ...
      const remainingNetworkRequestBytes = authority.maxTotalNetworkRequestBytes - session.committedNetworkRequestBytes;
      const remainingNetworkResponseBytes = authority.maxTotalNetworkResponseBytes - session.committedNetworkResponseBytes;
      if (remainingNetworkRequestBytes < 0 || remainingNetworkResponseBytes < 0) {
        fail("Browser session network budget was exhausted.", "BROWSER_AUTOMATION_BUDGET_EXCEEDED");
      }
      ...
      try {
        rawResult = await withinDeadline((signal) => backend.runAction(session.handle, action, Object.freeze({
          ...
          networkBudget: Object.freeze({
            maxRequestBytes: Math.min(authority.maxNetworkRequestBytes, remainingNetworkRequestBytes),
            maxResponseBytes: Math.min(authority.maxNetworkResponseBytes, remainingNetworkResponseBytes)
          }),
      ...
      let result;
      try {
        result = normalizeBackendResult(rawResult, action, policy.maxOutputBytes, Object.freeze({
          maxRequestBytes: Math.min(authority.maxNetworkRequestBytes, remainingNetworkRequestBytes),
          maxResponseBytes: Math.min(authority.maxNetworkResponseBytes, remainingNetworkResponseBytes)
        }));
      } catch {
      ...
      session.committedNetworkRequestBytes += result.attestation.network.requestBytes;
      session.committedNetworkResponseBytes += result.attestation.network.responseBytes;
      return Object.freeze({
        ...
        networkBudget: Object.freeze({
          committedRequestBytes: session.committedNetworkRequestBytes,
          maximumRequestBytes: authority.maxTotalNetworkRequestBytes,
          committedResponseBytes: session.committedNetworkResponseBytes,
          maximumResponseBytes: authority.maxTotalNetworkResponseBytes
        }),
        ...result
      });
    };
