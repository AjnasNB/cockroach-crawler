...
export async function installNetworkBoundary(session) {
  ...
  await requiredMethod(context, "route", "session-open")("**/*", async (route) => {
    const request = requiredMethod(route, "request", "session-open")();
    const rawUrl = requiredMethod(request, "url", "session-open")();
    let parsed;
    try {
      parsed = new URL(rawUrl);
    } catch {
      await requiredMethod(route, "abort", "session-open")("blockedbyclient");
      return;
    }
    const permittedOrigin = session.allowedOrigins.includes(parsed.origin);
    const lease = session.network.activeLease;
    const method = requiredMethod(request, "method", "session-open")().toUpperCase();
    const leaseActive = session.network.activeLease === lease
      && lease?.active === true
      && !lease.signal?.aborted
      && (!lease.deadline || Date.now() < Date.parse(lease.deadline));
    if (!leaseActive) {
      session.network.blocked += 1;
      await requiredMethod(route, "abort", "session-open")("blockedbyclient");
      return;
    ...
    if (lease.requests > session.network.maxRequestsPerAction
      ...
      session.network.blocked += 1;
      session.network.lastDeniedCode = "BROWSER_AUTOMATION_RESOURCE_LIMIT";
      session.network.lastDeniedUrl = safeUrl(parsed.toString());
      await requiredMethod(route, "abort", "session-open")("blockedbyclient");
      return;
    }
    if (!READ_ONLY_METHODS.has(method) && lease.effect !== "write") {
      session.network.blocked += 1;
      session.network.lastDeniedCode = "BROWSER_AUTOMATION_EFFECT_DENIED";
      session.network.lastDeniedUrl = safeUrl(parsed.toString());
      await requiredMethod(route, "abort", "session-open")("blockedbyclient");
      return;
    }
    const authorization = permittedOrigin
      ? await session.services.authorizeRequest(serviceInput(session, {
        sessionId: session.publicSessionId,
        url: parsed.toString(),
        origin: parsed.origin,
        method,
        resourceType: requiredMethod(request, "resourceType", "session-open")(),
        action: lease.action,
        effect: lease.effect,
      ...
      : null;
    const leaseStillActive = session.network.activeLease === lease
      && lease.active === true
      && !lease.signal?.aborted
      && (!lease.deadline || Date.now() < Date.parse(lease.deadline));
    if (!leaseStillActive) {
    ...
    if (!permittedOrigin || authorization?.allowed !== true) {
      session.network.blocked += 1;
      session.network.lastDeniedCode = "BROWSER_AUTOMATION_ORIGIN_VIOLATION";
      session.network.lastDeniedUrl = safeUrl(parsed.toString());
      await requiredMethod(route, "abort", "session-open")("blockedbyclient");
      return;
    }
