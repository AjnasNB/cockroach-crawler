...
const INTENTIONALLY_UNSUPPORTED = new Set([
  ...
  "worker.evaluate", "download", "popup.wait", "network.offline", "network.route.add", "network.route.remove",
  "network.headers"
]);
...
function trackGuardTask(session, task) {
  session.pendingGuards.add(task);
  task.finally(() => session.pendingGuards.delete(task)).catch(() => {});
  const lease = session.network.activeLease;
  if (lease?.active && lease.inflight) {
...
function attachPage(session, page) {
  ...
  if (!session.pageIds.has(page) && session.pages.size >= MAX_PAGES_PER_SESSION) {
    const task = Promise.resolve()
      .then(() => optionalMethod(page, "close")?.())
      .catch(() => { session.violationCode = "BROWSER_AUTOMATION_CLEANUP_UNCONFIRMED"; })
      .then(() => quarantine(session, "page resource limit exceeded"));
    trackGuardTask(session, task);
  ...
  if (typeof page?.on === "function") {
    page.on("popup", (popup) => {
      try {
        ...
        const task = Promise.resolve()
          ...
          .catch(async () => {
            session.violationCode = "BROWSER_AUTOMATION_CLEANUP_UNCONFIRMED";
          })
          .then(() => quarantine(session, "disallowed popup was created"));
        trackGuardTask(session, task);
      } catch {
        const task = quarantine(session, "popup resource limit exceeded");
        trackGuardTask(session, task);
      }
    });
    page.on("download", (download) => {
      const task = discardDownload(download)
        .catch(() => { session.violationCode = "BROWSER_AUTOMATION_CLEANUP_UNCONFIRMED"; })
        .then(() => quarantine(session, "browser download event is unsupported"));
      trackGuardTask(session, task);
    });
    ...
    page.on("framenavigated", (frame) => {
      ...
      try {
        ...
        trackGuardTask(session, quarantine(session, "asynchronous page origin violation"));
      }
    });
  }
}
...
function assertSingleOrigin(input) {
  ...
  const budget = input.networkBudget;
  for (const field of ["maxRequestBytesPerAction", "maxResponseBytesPerAction", "maxTotalRequestBytes", "maxTotalResponseBytes"]) {
    if (!Number.isSafeInteger(budget?.[field]) || budget[field] < 0) {
      browserAutomationFail("BROWSER_AUTOMATION_BACKEND_INVALID", "The governed session requires explicit non-negative network byte budgets.");
    }
  }
  if (budget.maxRequestBytesPerAction > budget.maxTotalRequestBytes
    || budget.maxResponseBytesPerAction > budget.maxTotalResponseBytes) {
    browserAutomationFail("BROWSER_AUTOMATION_BACKEND_INVALID", "Per-action network byte budgets cannot exceed session totals.");
  }
}
...
export function createGovernedPlaywrightBackend(options) {
  ...
  async function openSession(input) {
    ...
    if (!opened || typeof opened !== "object" || !opened.context || opened.ownedContext !== true
      || opened.networkIsolation?.serviceWorkers !== "block"
      || opened.networkIsolation?.webSockets !== "block"
      || opened.networkIsolation?.nonRoutedEgress !== "host-blocked") {
      const cleanup = [];
      if (opened?.context) {
    ...
    const session = {
      ...
      network: {
        ...
        readOnlyMethods: new Set(["GET", "HEAD", "OPTIONS"]),
        totalRequestBytes: 0,
        totalResponseBytes: 0,
        maxTotalRequestBytes: input.networkBudget.maxTotalRequestBytes,
        maxTotalResponseBytes: input.networkBudget.maxTotalResponseBytes
      },
      services,
      quarantined: false,
    ...
    session.registerWorker = (value) => {
      ...
      if (session.workers.size >= MAX_WORKERS_PER_SESSION) {
        trackGuardTask(session, quarantine(session, "worker resource limit exceeded"));
        return null;
      }
    ...
    session.registerFrame = (value) => {
      ...
      if (session.frames.size >= MAX_FRAMES_PER_SESSION) {
        trackGuardTask(session, quarantine(session, "frame resource limit exceeded"));
        return null;
      }
      ...
      if (typeof session.context?.on === "function") {
        session.context.on("page", (page) => {
          try {
            ...
            session.violationCode = "BROWSER_AUTOMATION_ORIGIN_VIOLATION";
            trackGuardTask(session, quarantine(session, "new page origin violation"));
          }
        });
      }
  ...
  async function runAction(session, action, execution) {
    ...
    const networkLease = {
      ...
      requestBytes: 0,
      responseBytes: 0,
      maxRequestBytes: execution?.networkBudget?.maxRequestBytes ?? 0,
      maxResponseBytes: execution?.networkBudget?.maxResponseBytes ?? 0,
      active: true,
      accepting: true,
      inflight: new Set(),
      controllers: new Set()
    ...
    networkLease.accepting = false;
    while (networkLease.inflight.size > 0) {
      await Promise.allSettled([...networkLease.inflight]);
    }
    networkLease.active = false;
    for (const controller of networkLease.controllers) controller.abort();
    if (session.network.activeLease === networkLease) session.network.activeLease = null;
    try {
    ...
    return Object.freeze({
      ...
      attestation: Object.freeze({
        ...
        network: Object.freeze({
          requests: networkLease.requests,
          requestBytes: networkLease.requestBytes,
          responseBytes: networkLease.responseBytes
        }),
