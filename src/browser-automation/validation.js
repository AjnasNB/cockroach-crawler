...
export function normalizeAuthority(value, { requireAuthorityId = false } = {}) {
  ...
  const normalized = Object.freeze({
    ...
    maxNetworkRequestBytes: integer(raw.maxNetworkRequestBytes, "Browser authority.maxNetworkRequestBytes", 0, 256 * 1024 * 1024),
    maxNetworkResponseBytes: integer(raw.maxNetworkResponseBytes, "Browser authority.maxNetworkResponseBytes", 0, 256 * 1024 * 1024),
    maxTotalNetworkRequestBytes: integer(raw.maxTotalNetworkRequestBytes, "Browser authority.maxTotalNetworkRequestBytes", 0, 1024 * 1024 * 1024),
    maxTotalNetworkResponseBytes: integer(raw.maxTotalNetworkResponseBytes, "Browser authority.maxTotalNetworkResponseBytes", 0, 1024 * 1024 * 1024)
  });
  if (normalized.maxNetworkRequestBytes > normalized.maxTotalNetworkRequestBytes
    || normalized.maxNetworkResponseBytes > normalized.maxTotalNetworkResponseBytes) {
    fail("Browser authority per-action network byte budgets cannot exceed session totals.");
  }
  return normalized;
}
