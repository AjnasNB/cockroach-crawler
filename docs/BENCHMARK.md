...
Cockroach Crawler `0.5.2` was evaluated against the full 511-page held-out split
...
npm run bench:public:wceb -- \
  ...
  --split test \
  --output bench/results/wceb-test-0.5.2.json
```
...
npm run bench:public:conformance -- \
  --output bench/results/public-conformance-0.5.2.json
npm run bench:public:verify
```

## Evidence files

- `bench/results/wceb-test-0.5.2.json`: aggregate metrics and all 511 page rows.
- `bench/results/public-conformance-0.5.2.json`: every robots and WPT case.
