...
- The public extraction-quality results use WCEB v1.0 at commit `62ff86d12ea72c80c31fb810ff1a724fad687bea` under CC-BY-4.0. The dataset is downloaded separately and is not committed or packed. Cockroach Crawler's evaluator and machine-readable derived metrics are original project code and output.
- WCEB calls the 511-page partition `test`, but this project previously inspected and iterated against it. The `wceb-core-observed-0.7.0.json`, `wceb-quality-observed-0.7.0.json`, `wceb-quality-fail-closed-observed-0.7.0.json`, and `extraction-comparison-0.7.0.json` artifacts therefore identify it as observed development evidence, not untouched held-out evidence. `wceb-quality-development-0.7.0.json` records the separate 1,497-page upstream development split.
- `cockroach-crawler/quality` calls the exact MIT-licensed `trafilatura@0.2.0` npm package. That package selects a separately installed native binary for supported Windows, macOS, or glibc Linux x64/ARM64 platforms. No upstream source or binary is copied into this repository or the Cockroach Crawler tarball, and core/serverless do not import the native backend.
- Committed WCEB evidence records the exact engine, boilerplate or quality profile, fail-closed state, and source fingerprints. The verifier rejects source mutations, profile mismatches, baseline manifest drift, or altered baseline output digests.
