...
This is the source-backed inventory for the stable `0.7.0` release. The npm
package, declarations, tests, documentation, and benchmark evidence are
reviewed together.

Status terms:

- **Stable 0.5.x/0.6.x** means the feature was introduced in an earlier stable npm line.
- **Stable 0.7.0** means the feature ships in the current stable npm release.
...
- **Stable 0.7.0:** an opt-in Node-only quality extraction subpath backed by
...
- **Stable 0.7.0:** `extractPageQuality` for supported Windows, macOS, and
