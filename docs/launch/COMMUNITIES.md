...
The product lesson behind the published Cockroach Crawler `0.7.0-rc.1` prerelease is that public URLs, provider APIs, logged-in read sessions, browser actions, quality extraction, and serverless fetches are not one capability. Each has a different setup, failure mode, and trust boundary.
