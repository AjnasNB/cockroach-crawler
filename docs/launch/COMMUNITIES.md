...
The product lesson behind stable Cockroach Crawler `0.7.0` is that public URLs, provider APIs, logged-in read sessions, browser actions, quality extraction, and serverless fetches are not one capability. Each has a different setup, failure mode, and trust boundary.
