...
1. Publish `0.3.0-alpha.3` through an npm trusted-publishing GitHub environment with provenance and `--tag next`; keep `latest` on stable `0.2.0`.
...
6. Create an annotated `v0.3.0-alpha.3` tag only at the exact green published commit. Attach only release-owned assets and generate `SHA256SUMS.txt` from exactly those attachments.
