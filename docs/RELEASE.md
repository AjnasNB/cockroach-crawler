...
Use this checklist from a clean, reviewed commit. Never publish from a worktree containing unreviewed changes or credentials. Stable `0.4.0` requires advanced capability, provider/security, packed-consumer, and exact-artifact approval on the reviewed release commit.
...
10. Exercise every `0.4.0` subpath from a packed consumer: strategies, cache,
...
## Stable 0.4.0 publication

1. Publish a fresh `0.4.0` artifact through the npm trusted-publishing GitHub environment with provenance and `--tag latest`; never move an alpha tarball onto the stable tag.
...
6. Create an annotated `v0.4.0` tag only at the exact green published commit. Attach only release-owned assets and generate `SHA256SUMS.txt` from exactly those attachments.
